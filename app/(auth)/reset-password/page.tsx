...
function ResetPasswordForm() {
  ...
  const handleSubmit = async () => {
    ...
    if (password.length < 10) {
      setError('Password must meet integrity metrics (minimum 10 characters).');
      return;
    }
    ...
    try {
      ...
      setError(err.message || 'Failed to reconfigure security key.');
    }
  };
  ...
  return (
    <Card className="w-full max-w-[420px] !p-8 z-10">
      ...
      {success && <p className="text-green-500 text-xs text-center mb-4">Security key reconfigured! Redirecting...</p>}
      ...
      <div className="flex flex-col gap-4">
        ...
        >
          {isLoading ? 'Syncing...' : 'Commit Changes & Sync'}
        </Button>
      </div>
    </Card>
  );
...
export default function ResetPasswordPage() {
  return (
    <div className="w-screen h-screen bg-bg-primary flex items-center justify-center relative px-4">
      ...
      <Suspense fallback={
        <Card className="w-full max-w-[420px] !p-8 z-10 flex items-center justify-center h-[300px]">
          <span className="text-sm text-text-secondary">Loading security module...</span>
        </Card>
      }>
        <ResetPasswordForm />
      </Suspense>
